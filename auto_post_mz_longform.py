...
# and crash the whole run. Use this narrower list for uploads until MZ is
# re-authed with refresh_token_mz.py.
...
LONGFORM_QUEUE_FILE = BASE_DIR / "mz_longform_queue.json"  # fixed Jul 5 2026 -- was reading the wrong file; channel_monitor.py writes mz_longform_queue.json
...
def generate_script(topic: str) -> dict:
    ...
    json_user = (
        ...
        f"- First sentence: the gut-punch fact or number (e.g. '$150M and 90 days — that's all that stood between Apple and total collapse.')\n"
...
def render_longform_video(script_data: dict, out_dir: Path) -> dict:
    ...
    if clip_paths:
        ...
        raise RuntimeError(
            "MZ long-form: no usable stock clips for "
...
_TITLE_DEDUP_STOPWORDS = {
    "the", "a", "an", "of", "in", "on", "at", "to", "for", "and", "or", "with", "how",
}
...
def _find_source_short(topic: str, queue: list) -> str | None:
    ...
    cross-link back to it. Two paths:
      ...
      2. Bank-sourced topics: fuzzy keyword-match against the channel's own
         Shorts log (mz_post_log.json) and use the best match above threshold.
    ...
    shorts_log_file = BASE_DIR / "mz_post_log.json"
...
def main() -> int:
    ...
    try:
        _qc_enforce(Path(render_result["video_path"]), channel="mz", kind="long")
