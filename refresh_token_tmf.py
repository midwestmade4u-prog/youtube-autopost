#!/usr/bin/env python3
"""
Re-authenticates the The Mind Files YouTube account and saves a fresh token.
IMPORTANT: Sign in with the account that OWNS the The Mind Files channel.
           Channel ID: UC0O6KbbHKW4_a7d9epNo93A  (The Mind Files)

Usage:
    python3 refresh_token_tmf.py

After it finishes, run:
    pbcopy < youtube_token_tmf.json
Then paste into GitHub Secret: YT_TOKEN_TMF
"""
...
TOKEN_FILE     = Path(__file__).parent / "youtube_token_tmf.json"
...
def main():
    ...
    print("Opening browser for The Mind Files OAuth…")
    print("→ IMPORTANT: Sign in with the account that owns The Mind Files channel")
    print(f"→ Expected channel ID after login: {TMF_CHANNEL_ID}\n")
    ...
    print("\nNext steps:")
    print("  1. pbcopy < youtube_token_tmf.json")
    print("  2. GitHub → youtube-autopost repo → Settings → Secrets → YT_TOKEN_TMF → Update")
    print("  3. Paste and save")
    print("  4. Go to GH Actions → YouTube Auto-Post - The Mind Files → Run workflow")
